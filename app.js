...
const translations = {
  en: {
    ...
    aiPromptBody: "Help me create a Decisn choice list for this goal: [describe what I am choosing]. Decisn is a local-first random picker that accepts comma-separated or line-separated options, optional weights like option:3, option:25%, option:1/365, and ranges like [1,10]. Suggest practical options and, when useful, recommend simple weights.",
  ...
  zh: {
    ...
    aiPromptBody: "请帮我创建一份可用于 Decisn（帮我选）的选项列表，目标是：[描述我要选择什么]。Decisn 是一个本地优先的随机选择器，支持逗号分隔或换行分隔的选项，也支持 选项:3、选项:25%、选项:1/365 这样的权重，以及 [1,10] 这样的范围。请给出实用选项；如果有帮助，也可以建议简单权重。",
    copyAiPrompt: "复制提示词",
